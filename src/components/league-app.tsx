...
export function LeagueApp() {
  ...
  const applyPayload = useCallback(
    (next: AppStateResponse, grade = currentGrade) => {
      setPayload(next);
  ...
  const category = useMemo(() => {
    ...
    return (
      payload.data[categoryKey(currentGrade, currentSport)] ||
      createInitialCategory(currentGrade, currentSport)
    );
  ...
  useEffect(() => {
    if (!payload) return;
    setDraftGroups(
      payload.data[categoryKey(currentGrade, currentSport)]?.groups || [],
    );
  }, [payload, currentGrade, currentSport]);
  ...
  const handleAuthClick = async () => {
    if (payload?.isAdmin) {
      ...
      setPayload((current) =>
        current
          ? { ...current, isAdmin: next.ok ? false : current.isAdmin }
          : current,
      );
  ...
  const verifyPin = async () => {
    try {
      ...
      setAuthOpen(false);
      setPayload((current) =>
        current ? { ...current, isAdmin: true } : current,
      );
      router.push("/admin");
  ...
  const patchLocalMatch = useCallback(
    (
      ...
      setPayload((current) => {
        ...
        if (matchId !== "sf1" && matchId !== "sf2" && matchId !== "final")
          return current;
        return {
          ...current,
          data: {
  ...
  return (
    <div className="flex min-h-screen flex-col justify-between bg-slate-50 text-slate-800 antialiased">
      <header className="sticky top-0 z-30 bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 text-white shadow-lg">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3.5 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="flex items-center gap-2 text-md font-extrabold tracking-tight sm:text-xl">
                학교스포츠클럽 점심리그
          ...
          <div className="flex items-center space-x-2">
            <div className="hidden items-center space-x-1.5 rounded-full border border-slate-600 bg-slate-700/80 px-3 py-1 text-xs font-medium text-slate-300 sm:flex">
              ...
              />
              <span>
                {payload.isAdmin ? "관리자 권한 활성" : "보기 전용 모드"}
              </span>
            </div>
            <button
      ...
      <section className="border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
            <span className="text-xs font-bold tracking-wider text-slate-500 uppercase whitespace-nowrap">
            ...
            {([1, 2, 3] as const).map((grade) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between gap-1.5 rounded-lg border px-3.5 py-2 text-xs transition-all sm:w-auto sm:justify-start sm:py-1.5 ${
                    active
      ...
      <nav className="border-b border-slate-200 bg-slate-100">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-x-1 sm:gap-x-4">
            {TABS.filter((tab) => tab.id !== "admin" || payload.isAdmin).map(
              (tab) => {
                const Icon = tab.icon;
      ...
      <main className="mx-auto mb-auto w-full max-w-7xl px-3 py-4 sm:px-6 sm:py-6 lg:px-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:mb-6 sm:gap-3 sm:p-4">
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-100 text-base font-bold text-indigo-700 sm:h-10 sm:w-10 sm:text-xl">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 sm:text-xs">
                조별 링크 대진(A➔B➔C➔D➔A), 경기일자 입력 및 3세트 점수 산출을
                지원합니다.
              </p>
            </div>
          </div>
          <span className="inline-flex items-center rounded-md border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 sm:px-2.5 sm:py-1 sm:text-xs">
        ...
        {currentTab === "finals" ? (
          <FinalsView
            ...
            onAutoFill={() =>
              ask(
                "본선 팀 자동 배치",
                "예선 성적을 바탕으로 준결승 대진표에 팀을 자동 배치하시겠습니까?",
                async () => {
                  try {
                    const next = await autoFillFinals({
                      grade: currentGrade,
                      sport: currentSport,
                    });
                    applyPayload(next);
        ...
        {currentTab === "admin" ? (
          <AdminView
                  ...
                  const next = await saveGroups({
                    ...
                    groups: draftGroups.map((group) => ({
                      ...
                      teams: group.teams.map(
                        (team, index) => team.trim() || `팀${index + 1}`,
                      ),
                    })),
                    regenerate: true,
                  });
            ...
            onReset={() =>
              ask(
                "데이터 초기화",
                "현재 학년/종목의 모든 데이터가 초기화됩니다.",
                async () => {
                  const next = await resetCategory({
                    grade: currentGrade,
                    sport: currentSport,
                  });
                  applyPayload(next);
      ...
      <footer className="mt-12 border-t border-slate-800 bg-slate-900 py-6 text-xs text-slate-400">
        <div className="mx-auto max-w-7xl space-y-2 px-4 text-center">
          <p className="font-semibold text-slate-300">
            중학교 학교스포츠클럽 점심리그 관리 시스템
          </p>
          <p className="text-slate-500">
            학생들의 공정한 경기 운영 및 주도적 참여를 지원합니다.
          </p>
        </div>
      </footer>
