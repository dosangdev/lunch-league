...
export function LeagueApp() {
  ...
  const applyPayload = useCallback((next: AppStateResponse, grade = currentGrade, sport = viewSport) => {
    setPayload(next);
    const key = categoryKey(grade, sport);
    setDraftGroups(next.data[key]?.groups || []);
  }, [currentGrade, viewSport]);

  const load = useCallback(async () => {
    const next = await fetchLeague();
    setPayload(next);
    const sport = next.gradeSports[1];
    setViewSport(sport);
  ...
  const officialSport = payload?.gradeSports[currentGrade] || "ultimate";
  const currentSport = viewSport;
  ...
  const persistSports = async (gradeSports: GradeSports) => {
    const next = await saveGradeSports(gradeSports);
    applyPayload(next);
  };
  ...
  const switchGrade = (grade: 1 | 2 | 3) => {
    ...
    if (payload) setViewSport(payload.gradeSports[grade]);
  };

  const switchSport = async (sport: Sport) => {
    setViewSport(sport);
    setSelectedGroup("ALL");
    if (!payload?.isAdmin) return;
    const nextSports = { ...payload.gradeSports, [currentGrade]: sport };
    await persistSports(nextSports);
      ...
      <section className="border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div className="custom-scrollbar flex items-center space-x-2 overflow-x-auto pb-1 md:pb-0">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-xs font-bold tracking-wider text-slate-500 uppercase whitespace-nowrap">
                <Volleyball className="mr-1 inline h-3.5 w-3.5" />
                현재 학년 종목:
              </span>
              <div className="flex rounded-xl border border-slate-200 bg-slate-100 p-1">
                {(["ultimate", "volleyball"] as const).map((sport) => {
                  const active = currentSport === sport;
                  return (
                    <button
                      key={sport}
                      onClick={() => switchSport(sport)}
                      className={`flex items-center space-x-1.5 rounded-lg px-3.5 py-1.5 text-xs font-extrabold transition-all ${
                        active
                          ? sport === "ultimate"
      ...
      <main className="mx-auto mb-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <span className="inline-flex items-center rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">
            ...
            {officialSport === currentSport ? "자동 저장" : "보기 전환 중 (대표 종목과 다름)"}
          </span>
        </div>
        ...
        {currentTab === "admin" ? (
          <AdminView
            ...
            onSetGradeSport={async (grade, sport) => {
              const nextSports = { ...payload.gradeSports, [grade]: sport };
              await persistSports(nextSports);
